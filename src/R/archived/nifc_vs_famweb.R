...
costs <- fread('data/NIFC_Fam_Lise.csv', header = TRUE, stringsAsFactors = TRUE)

costs_p <- costs %>%
  ggplot() +
  geom_point(aes(x = year, y = nifc/1000000000), color = "#1F77B4", size = 3) +
  geom_point(aes(x = year, y = famweb/1000000000), color = "#2CA02C", size = 3) +
  geom_line(aes(x = year, y = nifc/1000000000), color = "#1F77B4") +
  geom_line(aes(x = year, y = famweb/1000000000), color = "#2CA02C") +
  geom_smooth(aes(x = year, y = nifc/1000000000), method="glm", method.args = list(family = "poisson"),
              se= FALSE, size = 0.75, color = "#1F77B4") +
  geom_smooth(aes(x = year, y = famweb/1000000000), method="glm", method.args = list(family = "poisson"),
